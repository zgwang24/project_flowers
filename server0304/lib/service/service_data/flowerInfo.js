...
// read orderIds
FlowerInfo.prototype.readOrderIds = function(orderIdAndGetPhone, callback){
	flowerInfoModel.find({}, function(err, res){
		if(err){
			...
			if(res == null){
				...
				var array = new Array();
				var arrayGetPhone = new Array();
				for(var i = 0; i < res.length; i++){
					for(var j = 0; j < res[i].wishes.length; j++){
						array.push(res[i].wishes[j].orderId + res[i].wishes[j].getPhone);
					}
				}
				var result = array.indexOf(orderIdAndGetPhone);
				callback(err, result);	
			}
		}
	})
...
// update wish
FlowerInfo.prototype.updateWish = function(getPhone, orderId, obj, callback){
	flowerInfoModel.find({phone: getPhone}, function(err, res){
		if(err){
			console.log(err);
		}else{
			var wishes = new Array();
			for(var i = 0; i < res.length; i++){
				if(res[i].phone == getPhone){
					for(var j = 0; j < res[i].wishes.length; j++){
						var item = res[i].wishes.shift();
						if(item.orderId == orderId){
							
						}else{
							res[i].wishes.push(item);
						}
					}
					res[i].wishes.push(obj.wishes[0]);
					console.log(res[i].wishes);
					res[i].save(callback);
				}
			}   
		}
